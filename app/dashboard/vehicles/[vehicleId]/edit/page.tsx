...
export default function EditVehiclePage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/vehicles/${resolvedParams.vehicleId}`)
      .then(res => res.json())
      .then(data => {
        setFormData({
          name: data.name || "",
          ...
          oilChangeInterval: data.oilChangeInterval?.toString() || "3000",
          oilChangeIntervalMonths: data.oilChangeIntervalMonths?.toString() || "3",
        })
        if (data.imageUrl) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/vehicles/${resolvedParams.vehicleId}`, {
        method: "PATCH",
        headers: {
      ...
      router.push(`/dashboard/vehicles/${resolvedParams.vehicleId}`)
      router.refresh()
    } catch (error: any) {
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-gray-500">A friendly name to identify this vehicle</p>
            </div>
